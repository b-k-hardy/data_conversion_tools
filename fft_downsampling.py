...
def add_complex_signal_noise(imgfft, targetSNRdb):
    ...
    if add_complex_noise:
        ...
        # gauss_noise = rng.normal(0, sigma, imgfft.shape)
        gauss_noise_complex = rng.multivariate_normal(
            np.zeros(2), 0.5 * (sigma**2) * np.eye(2), size=imgfft.shape
        ).view(np.complex128)
        gauss_noise_complex = np.squeeze(
            gauss_noise_complex
        )  # for some reason the above line of code generates a singleton dimension

        imgfft += gauss_noise_complex
...
def downsample_phase_img(velocity_img, mag_image, venc, crop_ratio, targetSNRdb):
    ...
    # -----------------------------------------------------------
    new_complex_img, shifted_freqmag = downsample_complex_img(
        complex_img, crop_ratio, targetSNRdb
    )
    # -----------------------------------------------------------
